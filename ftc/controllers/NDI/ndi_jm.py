...
class NDIController(fym.BaseEnv):
    ...
    def get_control(self, t, env):
        ...
        E_h = np.diag(
            [
                float(-np.cos(ang[1]) * np.cos(ang[0]) / env.plant.m),
                env.plant.d / env.plant.J[0, 0],
                env.plant.d / env.plant.J[1, 1],
                env.plant.d / env.plant.J[2, 2],
            ]
        )
        ...
        M_h = np.vstack(
            (
                ...
                ang_dot[1]
                * ang_dot[2]
                * ((env.plant.J[1, 1] - env.plant.J[2, 2]) / env.plant.J[0, 0]),
                ang_dot[0]
                * ang_dot[2]
                * ((env.plant.J[2, 2] - env.plant.J[0, 0]) / env.plant.J[1, 1]),
                ang_dot[0]
                * ang_dot[1]
                * ((env.plant.J[0, 0] - env.plant.J[1, 1]) / env.plant.J[2, 2]),
            )
        )
